SUBROUTINE NewtonTumP1sommets(A, F, Uold, U, S, AA, ndim, choixf, temps)
    ...
    DO kiter = 1, kitermax
       ...
       DO i = 1, NsInt
          FF(i) = AireDsommet(i)*( Xk(i)-Uold(i) )/dt -AireDsommet(i)*( rate*Xk(i)*(1-Xk(i)) - apop*Xk(i) -&
          & radiotherapy(temps)*Xk(i) - chemotherapy(temps)*Xk(i))
          !! derivee de FF par rapport a Xk
          call ajout(i,i, AireDSommet(i)/dt, A )
          call ajout(i,i, - AireDsommet(i)*rate*(1-2*Xk(i)), A )
          call ajout(i,i, AireDSommet(i)*apop+AireDSommet(i)*radiotherapy(temps)+AireDSommet(i)*chemotherapy(temps),A)
       END DO
