...
contains
  ...
  subroutine plot_vtk (vec,chaine,nomchamps)
   ...
   Integer :: kt, nbs, nbt, uplotvtk
   character(len=30) :: FPLOTVTK 
   ...
   do kt = 1, NBS
      write (uplotvtk,*) CoordS(kt,1), CoordS(kt,2), 0.D0 
   end do

   write(uplotvtk,*) 'CELLS ',Nbt, 4*Nbt

   do kt=1,Nbt
      write (uplotvtk,*) 3,NuSoK(kt,1)-1, NuSoK(kt,2)-1, NuSoK(kt,3)-1 ! NumSoK
   end do
